...
If you have files written for 2.x, `migrate` is not optional. Most of what changed at 3.0 is loud, and a file carrying it says so the first time it loads. One part is not loud: a `#` needs a space in front of it to open a comment now, and 2.x needed none, so `url: http://h/#frag` used to stop at the `#` and no longer does. Both readings are legal text, both load clean, and `check` says `ok` either way, so nothing but `migrate` can tell you a file's values changed meaning. Run it once over each 2.x file before anything writes to that file, and read what comes back. The one shape it cannot carry across is a raw block's label holding a space and a `#`, since that `#` now ends the label.
