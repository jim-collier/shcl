...
##		                         laid down (binary, symlink, code/, scripts/),
##		                         and nothing else.
##
...
if (( uninstall )); then
	...
	${asroot} rm -f "${dest}/code"/* "${dest}/scripts"/* 2>/dev/null || true
	${asroot} rmdir "${dest}/code" "${dest}/scripts" "${dest}" 2>/dev/null || true
	echo "removed"
	exit 0
fi
...
if [[ -n "${want_src}" ]]; then
	...
	mkdir -p "${tmp}/x" "${tmp}/code" "${tmp}/scripts"
