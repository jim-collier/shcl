...
- `migrate` ignores a `Format` line inside a raw body, which is the block's content and not the file's version. C `migrate` also finds the line in a file that starts with a BOM.
