...
- 🔘 Seven separate scanners each decide where a line's parts begin and end, and every scanner item since July was two of them disagreeing.
	...
	- Note: after the funnel item, before the setter and 3.0 items. Additive, so it fits a 2.x release.
	- Opened: 20260906-090943
...
- 🔘 Each setter carries its own trim, carriage-return and `#` rule, and twelve items were a setter writing text the next load reads differently.
	...
	- Note: after the tokenizer item. Additive, so it fits a 2.x release, and it does not wait for 3.0.
	- Opened: 20260906-090943

- 🔘 3.0: cut the lexical rules down to a set that fits in a sentence each, with a formatter in the last 2.x that migrates a file.
	...
	- Fix: the migration. The last 2.x release carries a formatter mode that rewrites a 2.x file so the 3.0 parser reads the same tree: a bare value or element carrying a backslash escape is double-quoted, a single-quoted piece carrying an escape becomes double-quoted, a `#` that 2.x read as a comment gets a space before it, and the sugar is already gone in block form. Gate: for every corpus input and fuzz document, the 3.0 parse of the migrated text equals the 2.x parse of the original, as canonical text and as reads. The 2.x side runs from the last tag's binary.
	- Note: the compromise, stated once. A bare `\n` or `\t` and a single-quoted escape change meaning at 3.0. One formatter pass migrates a file, and a 2.x reader is unaffected by the migrated file. Everything else in the list is either already the rule or closer to what TOML and YAML users expect than what 2.x does.
	- Note: after the tokenizer item and after the 2.1.0 cut. The three items above are additive and go out in 2.x first.
	- Opened: 20260906-090943
