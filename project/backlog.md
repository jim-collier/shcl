...
- 🛠️ A CI/CD pipeline kicked off by a bash script (`cicd/cicd.bash`): builds, tests, and can commit and push. Packaging and publishing are opt-in.
	...
	- ✅ Cross-binding differential check (`cicd/utility/crosscheck.bash`) wired after tests: all binding CLIs must agree byte-for-byte (stdout + exit code) on `fmt` over every corpus input, every `reads.tsv` row, and a fuzz-dumped input set (`SHCL_FUZZ_DUMP`). No-op note with one binding; activates automatically when a second lands in `BINDING_CLIS`.
