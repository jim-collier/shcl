...
- **20260817**:
	...
	- **Improvements**:
		...
		- ✅ Names compare by their escaped spelling, so two spellings of the same name are two names. DONE: escapes resolve on names, as of the coming major.
			...
			- The version identity is staged on its own branch, not merged: 2.0.0 across the eight bump files, the Go module path moved to `source/go/v2` (go.mod, the CLI import, and every README reference), the per-binding dependency constraints, and the changelog entry for everything since 1.2.0. What is deliberately NOT done there is the part that cannot be taken back cheaply - no main merge, no tags, no signature, no registry publish.
