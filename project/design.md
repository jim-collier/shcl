...
- **A save can keep the lines no edit touched.** Among a second save that keeps the file's own text (kdl-rs keeps a document's formatting and has an `autoformat` beside it), working out the smallest text edit for each change and splicing it in (how VS Code's jsonc-parser edits `settings.json`), and canonical output only, it was decided to take the first two together: a separate save the program asks for, built by splicing (the user, 2026-09-25). The plain save and `fmt` stay canonical.
	...
	- The result has to reload as the edited document, with nothing lost and no error the file did not already have. When it does not, the save writes the canonical form and says so (the user, 2026-09-25). That check is what lets the line-up be simple: it decides how much of the file stays as it was, never what the file says.
	...
	- What falls back today: a child added under a flat dotted line or under a list written as `*` lines, and a value changed where a selector elsewhere names it. So does any edit to a file whose canonical order is not its own, as with blocks folded together from separate places or a selector that puts a child under an earlier block. Keeping some of those lines would move the rest.
