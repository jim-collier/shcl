...
- Name identity will normalize by escape resolution, at the next major and not before. Today a name is compared and emitted in its escaped spelling, so `"a\"b"` and `'a"b'` are two fields while those same two spellings as values are one string - the two halves of the language disagree on what a quoted string means, and a consumer splicing user text into a path has to know which half it is in. Among the options it was decided to resolve escapes on names: they already normalize once (ASCII case folds), so this finishes a rule the language started rather than inventing one. It waits for a major because it moves canonical output for a spelling already published, and because two fields that are distinct today would merge - the same class of merge case folding already performs. `QuoteSegment` is the mitigation until then. The cost is a version number, not a migration: crates.io and PyPI carry a 2.x beside a 1.x with nothing to do, and only Go pays anything real, since v2 goes in the module path and every Go consumer edits an import line.
