...
- `#` begins a comment that runs to end of line, **only when outside quotes** and only when it is first on the line (after the indent) or preceded by a space or tab. That is YAML's rule: `url: http://h/#frag` is a bare value, `a[#2]` is an index selector, `c#` is a label, and `a:#x` is the value `#x`.
...
- A `#` inside quotes is literal (`url: "http://h/#frag"`), a `#` with no whitespace before it is content (`x#y`), and a `#` inside a raw block is literal.
...
- Inside **double quotes** only, `\` triggers escape processing: `\t` (tab), `\n` (newline), `\\` (backslash), `\"`, `\'`; any other pair stays as written. Single quotes are literal, and bare text never processes a backslash, so `path: C:\dir\file` and `a[C:\].b` are plain text. This is TOML's and YAML's rule.

- The same rule applies to a **field name**, so a name is identified by what it spells rather than how it was spelled: `"a\"b"` and `'a"b'` are the same field. Canonical output picks one spelling and escapes it, and `QuoteSegment` produces that spelling, so any name round-trips. The as-authored accessor is the one place the source spelling is handed back (see the Accessor).
...
- **Same-line spelling:** the fence may also open on the field's own line (`path.name: ~~~sql` ... close fence). Identical tree; the child-indent spelling is canonical. The info-string rides the fence line in both spellings and is read as bare text: a `#` with no whitespace before it is part of the label (`db: ```c#` labels the block `c#`), and a `#` after a space or tab opens the line's trailing comment, in either spelling (`~~~sql  # note` labels the block `sql`). Canonical output puts that comment on the field's own line.
...
| `E017` | a value element or a selector body opens a quote it never closes with the matching quote as its last character - the piece is read bare, quotes and all; a comma or comment after it still ends it. In a field name the same typo is `E014`, since a bare name cannot carry a quote
...
| `E019` | a value beginning with `[`, the way JSON, TOML and YAML spell an array (`ports: [80, 443]`). A `[` after the colon is never a selector, and reading the text without its brackets would bake a changed value in, so the line is kept verbatim like any other malformed line: it binds nothing, a read on it is `NotFound`, and nothing is counted lost. An array is written without brackets (`ports: 80, 443`)
...
- The **Writer** handles the reverse of the Accessor: emit values, defaults, and comment sections, and canonicalize a file (see below). It mirrors the Accessor's typed-entry-point shape - a `Set<T>` per type (`SetInt`/`SetString`/.../`SetRaw`) and their array forms - so a programmatic value comes out as canonical text with no consumer-side formatting.
	- Each setter is the exact inverse of the matching read: `SetString` re-quotes and escapes so the value reads back verbatim; `SetFloat`/`SetInt` emit the same canonical number text the reader accepts; `SetDateTime` stores the canonical spelling; `SetRaw` picks a fence long enough that the content cannot close it early, trims the info-string the way a fence line reads it back, and refuses an info-string holding a line break or a `#` after whitespace (the emitted fence line would read the `#` as opening a trailing comment, so the block could not round-trip). The same refusal covers any value with no spelling the reader accepts: `SetFloat` fails on an infinity or a NaN, and `SetDateTime` on a value the reader would not parse back to the same fields (a month of 13, a fraction with no seconds, an offset past a day, an empty value). A refused write binds nothing and returns false.
	- A writer-created **top-level** node carries the blank-line grouping a hand-written file would have (one blank line between top-level sections; never as the first line), so writer output and hand-authored examples agree on shape.
	...
	- Companions round out the surface: `Set<T>Default` writes only when the path does not already resolve (the "emit defaults" half; every typed setter has one, `SetRaw` and `SetLiteral` included); `SetLiteral` takes the value as **syntax** rather than data - it reads its argument the way the parser reads the half of a line after the colon, so a caller holding value text writes `80, 443` as a two-element array without having to know its shape first (rejecting what could not be one line's value: a line break or a carriage return, a piece that opens a quote it never closes, or text beginning with `[`, which a file refuses as `E019`; a `#` after whitespace ends the value as it would in a file; text a file line would not read as a value at all - a fence opener - is stored as the string it spells, since the call reads a value half, not a line); `Exists` reports presence; `SetComment` attaches a leading comment line (creating an empty node so a section can be annotated), placing a node's blank separator line above the comment so the comment sits against its node; and `Remove` deletes the node(s) at a path.
	- Every setter reports whether the write applied: an unusable path (a wildcard, a `[#index]` instance that does not exist, a value part, a `[value]` selector carrying a literal line break - which has no one-line spelling, since the value emitter does not escape one - or a path past the depth cap) applies nothing, no half-created intermediates, and reports failure, which the CLI reports as exit 1 rather than silently printing the untouched document. `WriteReason(path)` (each binding's spelling) names which of those a failed write hit - `Writable`, `BadPath`, `ValueInPath`, `Wildcard`, `NoSuchIndex`, or `TooDeep` - by running the same validation the setters run, creating nothing, so a consumer's error message need not guess.
	...
	- On the CLI, `set --write` (like `fmt --write`) rewrites FILE in place through a temp-file-and-rename in the same directory, so an interrupted write can never truncate the config it rewrites. `set --write` also **creates** FILE when nothing is at the path yet, so a first write needs no `touch` beforehand; `fmt --write` has nothing to format and still reports it missing, and a FILE that exists but cannot be read is an error either way, never something to quietly write over.
	- The rename publishes a new file, so FILE is resolved through symlinks first: a linked-in config is written through, not replaced; a link to a file that does not exist yet gets that file created where it points; and a symlink cycle is an error - too many levels of symbolic links - with nothing written. The file's mode is carried over, setuid, setgid and sticky bits included, as an editor's rewrite would carry it.
	...
	- A path that names a directory rather than a file - it ends in a separator, or its last component is `.` or `..` - is refused: a document is not a directory, and the platforms disagreed about whether such a path even reached the filesystem.

	- Any other hard link to the old file keeps the old content. That is inherent to publishing a new file in the old one's place, and so is everything else a file carries outside its permission bits: POSIX ACLs, extended attributes and the SELinux label among them, and on Windows a file's own explicit ACEs and alternate data streams. Ownership is not kept either: the new file belongs to whoever ran the save, which only shows when that is not the old file's owner. In a sticky directory such as `/tmp` the rename is refused for a file the saver does not own, and the save reports it.
	- A read-only file is rewritten, since the rename needs the directory rather than the file, and the new file comes back read-only; Windows clears the attribute for the publish and sets it again after, so the platforms agree. Windows closes most of the carry-over gap where it can: the publish goes through `ReplaceFile`, which carries the destination's ACLs, security attributes and named streams onto the replacement, and falls back to a plain replacing move when the file is being created or the merge cannot be done. Neither carries the basic attributes, so hidden and system are re-applied after the publish along with read-only. On POSIX the containing directory is synced after the rename, as the file is before it, so a power cut cannot lose the publish itself and leave the old content in place.
...
Every consumer that persists a config re-implements the same load/save dance, and file lifecycle is where those consumers' bugs live - so the library carries a small optional file tier rather than leaving each caller to fumble it independently.
...
- `--set-literal=PATH=TEXT` is the same option with the other reading. `TEXT` goes in as **value syntax**, the way a file spells it, so that same text stores a two-element array. `--remove=PATH`, `--set-default=PATH=VALUE` and `--set-literal-default=PATH=TEXT` complete the edit options, and all five share one ordered list, so the last one to touch a path wins. On the read side, `--slots` prints an array read's per-slot statuses beside its values.
...
- A must-exist field whose path contains a wildcard is generated in dotted form (the wildcard dropped, targeting the instance it materializes) when some other live line materializes the wildcard's parent - otherwise the very instance that line creates would fail the schema. When that parent's line carries a value, a dotted child would name the empty-valued instance instead of the one the line made, so any line under a valued live parent selects it by the value: `srv: web` is followed by `srv[web].port:`, not `srv.port:`. Remaining wildcard paths, and paths that cannot be written at all (a name-wildcard `*` segment has no name to drop to; `[#N]` needs a pre-existing instance and its `#` would start a comment; a selector carrying a literal newline has no one-line spelling), are collected into a trailing `# Paths needing an instance name (not generated):` comment block, one `#   <path>   <type>` per line. A path that must exist and cannot be written at all is a `V097` fault naming the path instead, since the trailing block can never satisfy it - except a `repeat` lower bound of 2 or more, which is the documented shortfall below.
