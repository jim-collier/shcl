...
- **One element per line**, each a single scalar typed on read exactly like an inline element; quote to embed a comma, or to keep an element spelled like a field binding from hinting (`* "Bond, James"`). A bare comma on an element line is an error, not a second element.
...
- After the last field, and after the trailing block if there is one, a footer names the format and points at its spec, separated from what precedes it by one blank line:
	...
	These bytes are part of the generated file, so they are a cross-binding contract like the annotation line. The `Legal` line names SHCL as its subject: it says nothing about the config it sits in. The footer is written unless the caller asks for it to be left out - `no_banner` on the library call, `--no-banner` on `init` - and the flag is negative so a caller that says nothing gets the footer. Every binding exposes the same bytes as a constant (`GEN_BANNER`, `GenBanner` in Go, `SHCL_GEN_BANNER` in C), so a program writing its own config file writes the block without keeping a second copy of it. `SetBanner` puts the block at the end of a document and takes an old one off the footer first, found by its first text line or its `Format` line rather than its links, so a program that refreshes the block on each save never ends up with two. It is the only difference the flag makes: everything above it is byte-for-byte identical either way. The `Syntax` link points at the spec as tagged by the first release of the format major (`v3.0.0-beta1` for format 3), so a file keeps pointing at the rules it was written for. It changes only when the `Format` line does.
...
Which file it is looking at is the one thing `migrate` cannot read off the text. `p: 'C:\temp'` is spelled the same way under both rule sets and means two different things, so rewriting a 3.0 file would change what it says. The info block carries a `Format` line naming the format's major, and that is what decides. A file carrying it has nothing to migrate. One inside a raw body is that block's content and does not count. Where a file names more than one format, the highest decides. A file without it keeps those spellings as written and exits 7, unless `--from-2x` says it really is 2.x. A file `migrate` rewrites is stamped with that line, plus a note saying where it came from, so a second run has the answer the first one did not. Only `migrate` reads the line; every other command treats it as the comment it is. A program asks the same question with `FormatVersion` (each binding's spelling), and `MigrateUnstamped` is the rewrite without the stamp, for a program that writes the info block itself.
