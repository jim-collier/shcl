...
| `E003` | selector names an instance that does not exist: `a[5].b` where there is one `a`. The `[#N]` spelling cannot reach it from a file - the `#` opens a comment before the selector is read, so `b[#5].c: x` reports `E014` (empty selector) instead, and quoting the `#` makes it an ordinary discriminator - but the bare index spelling is documented and does reach it. On a write path the same situation comes back as the `NoSuchIndex` write reason rather than as a diagnostic
...
| `E019` | a value spelled with brackets, the way JSON, TOML and YAML spell an array. The brackets are read as a selector, so `ports: [80, 443]` folds two elements into the one string `80, 443`: an `error` when the brackets hold an unquoted comma, counted as lost content so an in-place rewrite refuses (`--lossy` overrides) rather than baking the changed value in. With no unquoted comma the value reads the same with or without the brackets (`tags: [prod]` is `tags: prod`) and the spelling is the documented `field:[disc]` sugar, so it is a `hint` and nothing is counted lost
