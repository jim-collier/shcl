...
- A must-exist field whose path contains a wildcard is generated in dotted form (the wildcard dropped, targeting the instance it materializes) when some other live line materializes the wildcard's parent - otherwise the very instance that line creates would fail the schema. When that parent's line carries a value, a dotted child would name the empty-valued instance instead of the one the line made, so any line under a valued live parent selects it by the value: `srv: web` is followed by `srv[web].port:`, not `srv.port:`. A commented line under a commented valued parent selects it the same way, so uncommenting both still names one instance. A live line under a commented parent stays dotted, since selecting by value would make the optional parent exist. Remaining wildcard paths, and paths that cannot be written at all (a name-wildcard `*` segment has no name to drop to; `[#N]` needs a pre-existing instance and its `#` would start a comment; a selector carrying a literal newline is left to the block rather than spelled inline), are collected into a trailing `## Paths needing an instance name (not generated):` comment block, one `##   <path>   <type>` per line. A path that must exist and cannot be written at all is a `V097` fault instead, since the trailing block can never satisfy it - except a `repeat` lower bound of 2 or more, which is the documented shortfall below. Every such path gets its own fault line, naming the path and why it cannot be written, so one run says how much of the schema is unreachable rather than one path at a time.

- A path whose last segment selects by value is written without that selector when its field has a `default`, because a value after a last-segment selector is ignored (`E002`). `field: "env[prod]"` with `default: prod` generates `env: prod`, which is the line that makes the instance the path selects. Whether the default names that instance is left to validation, so a default naming another one is a `V097` fault. With no `default` the selector line is written as it is (`flag[on]:`). A commented-out line follows the same rule.
...
Which file it is looking at is the one thing `migrate` cannot read off the text. `p: 'C:\temp'` is spelled the same way under both rule sets and means two different things, so rewriting a 3.0 file would change what it says. The info block carries a `Format` line naming the format's major, and that is what decides. A file carrying it has nothing to migrate. One inside a raw body is that block's content and does not count. Where a file names more than one format, the highest decides. A file without it keeps those spellings as written and exits 7, unless `--from-2x` says it really is 2.x. A file `migrate` rewrites is stamped with that line, plus a note saying where it came from, so a second run has the answer the first one did not. Only `migrate` reads the line; every other command treats it as the comment it is.
