...
- The **Writer** handles the reverse of the Accessor: emit values, defaults, and comment sections, and canonicalize a file (see below). It mirrors the Accessor's typed-entry-point shape - a `Set<T>` per type (`SetInt`/`SetString`/.../`SetRaw`) and their array forms - so a programmatic value comes out as canonical text with no consumer-side formatting.
	...
	- Companions round out the surface: `Set<T>Default` writes only when the path does not already resolve (the "emit defaults" half; every typed setter has one, `SetRaw` and `SetLiteral` included); `SetLiteral` takes the value as **syntax** rather than data - it reads its argument the way the parser reads the half of a line after the colon, so a caller holding value text writes `80, 443` as a two-element array without having to know its shape first (whatever a file line spells with that text is what gets stored, so a trailing blank comes off and a `#` behind a space or tab ends the value; what it refuses is what a file reports as an error, since a setter has no diagnostic to report it with - a line break, which no file line can hold, a piece that opens a quote it never closes (`E017`), and text beginning with `[`, which a file refuses as `E019`; text a file line would not read as a value at all - a fence opener - is stored as the string it spells, since the call reads a value half, not a line); `Exists` reports presence; `SetComment` attaches a leading comment line (creating an empty node so a section can be annotated), placing a node's blank separator line above the comment so the comment sits against its node, and refusing text that holds a line break; and `Remove` deletes the node(s) at a path.
	...
	- On the CLI, `set --write` (like `fmt --write`) rewrites FILE in place through a temp-file-and-rename in the same directory, so an interrupted write can never truncate the config it rewrites. `set --write` also **creates** FILE when nothing is at the path yet, so a first write needs no `touch` beforehand; `fmt --write` has nothing to format and still reports it missing, and a FILE that exists but cannot be read is an error either way, never something to quietly write over. A created file starts out as the info block `init` writes (see "Schema-driven generation"), so a new config says what format it is; the edits go above it, and `--no-banner` leaves it out. A file that already exists is never given one.
