...
Not yet modeled natively (as golden files): the on-bad/default outputs (covered cross-binding via the harness above, not by per-row `expected`), and diagnostic expectations (count, severity, code). The `check` diagnostic **codes** are already pinned cross-binding by the `load` rows; a `diags.tsv` could pin the exact code/line per case natively.
