...
- `expected-migrate.shcl` + `expected-migrate-diags.txt` (optional, as a pair) - the **migration** dimension. `expected-migrate.shcl` is the exact `migrate` output for `input.shcl`, byte for byte. `expected-migrate-diags.txt` is the load diagnostics of that output at Standard strictness, in `expected-diags.txt`'s format, so a rewrite that no longer loads cannot pass. Each binding runs its library `migrate` and matches both. No fixpoint is required, because migrate keeps the author's layout. The differential harness replays `migrate` across every CLI.
