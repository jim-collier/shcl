...
Case `026` pins schema-driven generation: `init-schema.shcl` carries `desc`/`default` on required and optional fields, an `allowed` set (rendered `one of: ...`), int and float ranges, a `repeat` bound, and a `server[*].host` wildcard. The golden `expected-init.shcl` shows required fields live (with their default or an empty value), optional fields commented out, the wildcard in a trailing comment block, and must itself load cleanly.
